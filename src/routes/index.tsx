...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "تعليم فني متطور يربط الدراسة بسوق العمل في تكنولوجيا الطباعة الرقمية والأوفست." },
    ],
  }),
  component: Home,
});
...
function Hero() {
  return (
    <section className="relative overflow-hidden bg-brand text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-28 grid lg:grid-cols-2 gap-10 items-center">
        <div>
          <Badge className="bg-[var(--accent-red)] hover:bg-[var(--accent-red)] text-white border-0 mb-4">
            وزارة التربية والتعليم — التعليم الفني التطبيقي
          </Badge>
          <h1 className="text-3xl sm:text-4xl md:text-6xl font-extrabold leading-tight">
            مدرسة مدحت السويدي <br />
            <span className="text-[var(--accent-red)]">للتكنولوجيا التطبيقية</span>
          </h1>
          <p className="mt-5 text-lg text-white/85 max-w-xl leading-8">
            تعليم فني متطور يربط الدراسة بسوق العمل. تدريب عملي داخل بيئة صناعية حقيقية، وشهادة معتمدة تفتح لك أبواب المستقبل.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Button asChild size="lg" variant="outline" className="border-white/40 text-white hover:bg-white/10 hover:text-white">
              <Link to="/admissions">شروط التقديم</Link>
            </Button>
          </div>
        </div>
...
function QuickStats() {
  const stats = [
    { icon: Calendar, label: "مدة الدراسة", value: "3 سنوات" },
    { icon: GraduationCap, label: "نظام الدراسة", value: "نظري وعملي وميداني" },
    { icon: Factory, label: "المجالات", value: "طباعة ديجيتال وأوفست" },
          ...
          <Card key={s.label} className="border-0 shadow-xl">
            <CardContent className="p-5 flex items-center gap-4">
              ...
              <div className="min-w-0">
                ...
                <div className="font-bold text-brand truncate">{s.value}</div>
              </div>
            </CardContent>
          </Card>
        ))}
...
function WhySection() {
  const items = [
    { icon: Factory, title: "تدريب عملي داخل بيئة صناعية", desc: "معامل حديثة وماكينات فعلية لتدريب واقعي." },
    { icon: TrendingUp, title: "ربط الدراسة بسوق العمل", desc: "مناهج مبنية على احتياجات الصناعة الحقيقية." },
    { icon: Wrench, title: "تأهيل مهني حقيقي", desc: "خريج قادر على العمل الفوري بعد التخرج." },
    { icon: Users, title: "دعم ومتابعة للطلاب", desc: "متابعة أكاديمية وسلوكية مستمرة." },
    { icon: Award, title: "فرص تدريب ميداني", desc: "تدريب داخل مصانع ومطابع كبرى." },
    { icon: ShieldCheck, title: "بيئة تعليمية منضبطة", desc: "قواعد ونظام يبني الشخصية والانضباط." },
  ];
  return (
    <section className="py-20 mt-10">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionTitle eyebrow="لماذا نحن؟" title="لماذا مدرسة مدحت السويدي؟" subtitle="ست ركائز تجعل من الخريج فنياً محترفاً جاهزاً لسوق العمل." />
...
function StudySystem() {
  const parts = [
    { pct: "40%", title: "تدريب عملي", desc: "داخل معامل وورش المدرسة" },
    { pct: "30%", title: "تدريب ميداني", desc: "داخل المصانع والمطابع" },
    { pct: "20%", title: "مواد نظرية تخصصية", desc: "أساسيات ومهارات التخصص" },
    { pct: "10%", title: "مواد ثقافية", desc: "لغات ومهارات عامة" },
  ];
  return (
    <section className="py-20 bg-secondary/60">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionTitle eyebrow="نظام الدراسة" title="ثلاث سنوات تصنع فنياً محترفاً" subtitle="توزيع متوازن بين النظري والعملي والميداني." />
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {parts.map((p) => (
            <Card key={p.title} className="text-center">
              <CardContent className="p-8">
                <div className="text-4xl font-extrabold text-[var(--accent-red)]">{p.pct}</div>
                <div className="mt-3 font-bold text-brand">{p.title}</div>
...
function ProgramsSection() {
  const icons: Record<string, typeof Printer> = {
    ...
    "design-montage": Palette,
    "marketing-sales": TrendingUp,
  };
  return (
    <section className="py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionTitle eyebrow="التخصصات" title="المجالات الدراسية" subtitle="تخصصات حالية ومستقبلية تواكب سوق العمل." />
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {PROGRAMS.map((p) => {
            ...
            return (
              <Card key={p.slug} className="relative overflow-hidden group">
                <CardContent className="p-6">
                  <div className="h-12 w-12 rounded-xl bg-secondary text-brand grid place-items-center mb-4 group-hover:bg-brand group-hover:text-white transition-colors">
                    <Icon className="h-6 w-6" />
                  </div>
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="font-bold text-brand">{p.title}</h3>
                    {p.status === "coming" && <Badge variant="secondary" className="text-[10px]">قريباً</Badge>}
                  </div>
                  <p className="text-sm text-muted-foreground leading-7">{p.short}</p>
...
function AdmissionSteps() {
  const steps = [
    ...
    { icon: UserCheck, t: "اجتياز مراحل القبول" },
    ...
    <section className="py-20 bg-brand text-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-10">
          ...
          <h2 className="text-2xl md:text-4xl font-extrabold">خطوات التقديم</h2>
          <p className="mt-3 text-white/75">خمس خطوات واضحة من التسجيل حتى القبول.</p>
        </div>
        <div className="grid gap-5 md:grid-cols-5">
          {steps.map((s, i) => (
            <div key={s.t} className="relative bg-white/5 border border-white/10 rounded-xl p-5 text-center">
              <div className="absolute -top-3 right-3 h-8 w-8 rounded-full bg-[var(--accent-red)] grid place-items-center text-sm font-extrabold">
                {i + 1}
              </div>
              <s.icon className="h-8 w-8 mx-auto text-white/90" />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="rounded-3xl bg-gradient-to-l from-brand to-[color-mix(in_oklab,var(--brand)_75%,black)] text-white p-8 md:p-12 grid md:grid-cols-[1fr_auto] items-center gap-6">
          <div>
            ...
            <p className="mt-3 text-white/80 max-w-xl">تعرف على نظام الدراسة، التخصصات، وشروط القبول من فريق المدرسة مباشرة.</p>
          </div>
